        ...
        try:
            ...
            for idx, feature in enumerate(features, 1):
                try:
                    ...
                    # Convertir strings vacíos a None o valores por defecto
                    ...
                    item = {
                        ...
                        "fecha": safe_string(attrs.get("fecha")),
                        "hora": safe_string(attrs.get("hora")),
                        ...
                        "departamento": safe_string(attrs.get("departamento")),
                        "ref": safe_string(attrs.get("ref")),
                    }

                    # Solo filtrar valores None de campos numéricos, mantener strings vacíos
                    item_to_insert = {}
                    ...
                    if len(item_to_insert) < 4:  # id, numero, timestamp + al menos 1 campo más
                        logger.warning(f"Sismo {idx} tiene muy pocos datos, se omite: {attrs}")
                        continue
                    
                    table.put_item(Item=item_to_insert)
                    inserted.append(item_to_insert)
                    logger.info(f"Sismo {idx} insertado con {len(item_to_insert)} campos")
                    
                except Exception as item_error:
                    logger.error(f"Error procesando sismo {idx}: {str(item_error)}, datos: {attrs if 'attrs' in locals() else 'N/A'}")
                    # Continuar con los demás sismos
                    continue

            logger.info(f"Proceso completado. Total insertados: {len(inserted)} de {len(features)} sismos recibidos")
